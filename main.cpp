...
void TestRelativeMove() {
  const Joints d = {500, 0, 0, 0, 0};
  for (int i = 0; i < 5; i++) {
    D5R_JointsMoveRelative(d);
    Sleep(1000);
  }
}

void TestSetAccumulate() {
  int result = D5R_SetAccumulateRelative(false);
...
int main(int argc, char* argv[]) {
  ...
  // printf(
  //     "Enter \'j\' for change jaw, \'k\' is the position before change
  //     jaw\n");
  // printf(
  //     "Enter \'f\' for fetch ring, \'g\' is the position before fetch
  //     ring\n");
